...
bool compare(const Point& a, const Point& b) {
    if (a.x != b.x) {
        return a.x < b.x; // x가 같으면 y가 작은 순서대로 정렬
    }

    return a.y < b.y; // x가 작은 순서대로 정렬
}
...
int main()
{
    ...
    vector<int> vec_origin = { 4,6,9,2,7,5,3,1,8,0 };
    ...
    // 오름차순 정렬
    sort(vec_origin.begin(), vec_origin.end());
    ...
    // 내림차순 정렬
    sort(vec_origin.rbegin(), vec_origin.rend());
	...
	// 순열 next_permutation()  // 시간복잡도 O(N*N!)
	...
	// max_element(), min_element()  // 시간복잡도 O(N)
