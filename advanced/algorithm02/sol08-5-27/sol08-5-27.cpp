...
// src 문자열로 만들 수 있는 depth길이만큼의 문자열 경우의 수
void combinataion1(string src, string dst, int depth) {
    if (dst.size() == depth) {
        cout << dst << endl;
    }
    else {
        for (int i = 0; i < src.size(); i++) {
            combinataion1(src.substr(i + 1), dst + src[i], depth);   // 재귀호출
        }
    }
}
...
vector<string> solution(vector<string> orders, vector<int> course) {
    ...
    for (int len : course) {
        ...
        // 주문 횟수가 2회 이상, 가장 많이 주문된 구성을 answer에 추가
        for (auto it : combi) {
