...
![](assets/20260805_111516_image.png)
...
트리 순회 - [소스](./advanced/algorithm02/sol09-4-28/sol09-4-28.cpp)

- 트리 순회 : 재귀호출로 처리. 왼쪽, 오른쪽 노드는 하위노드가 있을 수 있음
  - 전위 순회 : 부모노드(리턴) -> 왼쪽노드(재귀) -> 오른쪽노드(재귀) 순서로 탐색
  - 중위 순회 : 왼쪽노드(재귀) -> 부모노드(리턴) -> 오른쪽노드(재귀) 순서로 탐색
  - 후위 순회 : 왼쪽노드(재귀) -> 오른쪽노드(재귀) -> 부모노드(리턴) 순서로 탐색
  - 재귀호출을 순서대로 변경

![](assets/20260805_140823_image.png)

#### 모의 테스트

다단계 칫솔 판매 - [소스](./advanced/algorithm02/sol09-5-31/sol09-5-31.cpp)

https://school.programmers.co.kr/learn/courses/30/lessons/77486

1. 판매원 - 판매원 추천인 쌍으로 맵을 생성
2. 판매원 - 판매금액 담을 맵 생성
3. 판매인(seller)와 판매갯수(amount) 변수 값을 반복
4. 금액이 0보다 크고, 판매원 추천인이 없을 때까지(-) 반복하면서
5. amount * 100금액에서 10%를 뺀 금액을 판매원이 할당
6. 10% 분배금액은 추천인에게 보냄

- `제출 후 채점하기` 때 출력문 주석처리 필요. 테스트 통과 실패(출력문 갯수 초과)
- unordered_map, map 둘 다 문제해결에 관계 없음

![](assets/20260806_104509_image.png)

### 그래프

정점(vertex)과 간선(edge)으로 구성된 비선형 데이터구조. 비방향, 방향 구조 존재
...
- 인접 리스트 : 배열과 연결리스트(정점 v, 가중치 w 쌍) 조합 계속 연결

#### 그래프 탐색

그래프를 사용하는 이유 -> 탐색

- 깊이 우선탐색(DFS) - 스택, 재귀호출 > 모든 가능한 답을 찾는 백트래킹
- 너비 우선탐색(BFS) - 큐 > 최단경로, 네트워크 분석
    - 큐에 오름차순으로 push 

#### 그래프 최단경로 구하기

- 시작 Vertex에서 마지막 Vertex까지의 최단경로 : 간선의 수 또는 가중치 합의 최소

#### 다익스트라 알고리즘

- 양의 가중치로 간선이 구성된 그래프의 최단거리(최소비용) 구하는 알고리즘
...
1. 초기화는 다익스트라와 동일
2. 노드개수 - 1 반복 : 노드 갯수보다 많은 간선으로 구성된 그래프의 최단거리를 구하는 알고리즘
    - `간선의 수만큼 전부 최소비용 파악`
    - 시작노드로 갈 수 있는 각 노드에 최소비용을 확인해서 이전값(INF)과 갱신
3. 모든 간선으로 가능 경우를 전부 계산, 최소비용이 다시 갱신되면 `음의 순환` 존재하고 `문제를 풀 수 없음`. 음의 순환이 존재하지 않으면 문제를 해결할 수 있음

- 노드 5개 간선이 8개면, (총 네번 반복), 모든 간선의 수 8개의 최소비용 파악
